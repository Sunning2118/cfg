...
class AstNode:
    def __init__(self, gid, code=None, connectTo=None, child=None, d=None, u=None, isStart=False, isEnd=False):
        ...
        self.d = d if d is not None else []  # 定义
        self.u = u if u is not None else []  # 使用
...
class Graph:
    def __init__(self, ast, name="c"):  # Graph(ast, name="test")
        """
        ...
        :param name: 图名称
        """
        self.node_num = 0  # 图中节点的数量
        self.g = None  # 全局变量
        self.du_path = None  # du_path，定义使用路径
        self.dot = None  # 流程图类
        self.name = name  # 图名称
        self.build(ast)  # 构建AST树
    ...
    def getDeclTypeAttr(self, typeNode):  # 获取Decl得特性
    ...
    def getDecl_DU(self, declNode):  # getDecl_DU(eachNode)
    ...
    def getParamList_DU(self, ParamList):  # getParamList_DU(funcdecl.args) <pycparser.c_ast.ParamList object>
        # 获取函数形参中的du路径
        string = []
        d = []  # 定义
        u = []  # 使用
        ...
        for each in ParamList.params:
            each_str, each_d, each_u = self.getDecl_DU(each)  # 获取声明中的code和du路径
    ...
    def build_decl(self, nodeList):
        ...
        :param nodeList: [Node] pycparser全局变量节点
        :return: None
        """
        string = []
        d = []
        u = []
        n = AstNode(self.node_num)  # ast节点对象
        self.node_num += 1
    ...
    def build_nested_node(self, node, children, end, otherEnd=None, returnEnd=None, continueEnd=None):
        ...
        for i in range(len(children) - 1, -1, -1):
            ...
            if nodename == 'Switch':
                ...
                # 节点加入条件 和 子节点的dupath
                n, stmt_path = self.build_nested_node(n, children[i].stmt.block_items, end, otherEnd=end,
                                                      returnEnd=returnEnd)
    ...
    def build(self, node):  # 构建节点  build(ast)
        nodeName = node.__class__.__name__  # 构建AST树图
        if nodeName == 'FileAST':  # 开始节点
            self.g = []  # 全局变量
            self.du_path = []  # du路径
            self.dot = Digraph(name=self.name)  # 序列图
            flag = 0  # 初始为0
            decl = []
            typedef = []
            # funcdef = []

            for eachNode in node.ext:  # 下级节点列表
                # 变量声明
                ...
                # typedef数据类型定义
                    ...
                    if flag == 1:
                        ...
                        decl = []
                    flag = 2
                    # 把typedef声明放入typedef列表中
                    typedef.append(eachNode)
                # 函数声明
                elif eachNode.__class__.__name__ == "FuncDef":  # 先进函数
                    if flag == 1:
                        self.build_decl(decl)
                        decl = []
                    elif flag == 2:
            ...
            if flag == 1:
                ...
                self.build_typedef(typedef)
        elif nodeName == "FuncDef":  # 处理函数内部
            # FuncDef方法定义，有具体的实现过程
            # FuncDecl方法声明，先FuncDef定义后FuncDecl声明
            # 处理Decl节点下的 FuncDecl
            # quals = [str] （限定符号列表: const, volatile）
            # storage = [str] （存储说明符列表: extern, register, etc.）
            code = ''
            code += ' '.join(node.decl.storage) + ' ' if len(node.decl.storage) != 0 else '' + ' '.join(node.decl.quals) \
                                                                                          + ' ' if len(
                node.decl.quals) != 0 else ''

            funcdecl = node.decl.type  # 方法声明 <pycparser.c_ast.FuncDecl object at 0x000001C44D120B40>
            code_str, d, u = self.getParamList_DU(funcdecl.args)  # <pycparser.c_ast.ParamList object>
...
def build_graph(path, name="test"):  # build_graph(c_files[index], c_files[index].split('.')[0])
    # 处理‘#include’标签
    ...
    with open('tmp/c_processfile.c', 'w', encoding='utf-8') as f:
        f.write(txt)  # 写入待分析的路径
    # ast = parse_file('tmp/c_processfile.c', use_cpp=True, cpp_path=r'D:\mingw\bin\gcc.exe',
    #                  cpp_args=['-E', r'-I utils/fake_libc_include'])  # 生成AST
    ast = parse_file('tmp/c_processfile.c', use_cpp=False)  # 生成AST
    # use_cpp:如果希望在解析文件之前对其执行C预处理器，则设置为True。
    # cpp_path

    ast.show()
    print(ast)
    print(ast.__class__.__name__)  # FileAST
    print(ast.ext[0].__class__.__name__)  # FuncDef
    print(ast.ext[0].decl.__class__.__name__)  # Decl
    node = ast.ext[0]
    print(node.decl.__class__.__name__)  # Decl
    print(node.decl.type.__class__.__name__)  # FuncDecl
    print(node.decl.type.type.__class__.__name__)  # TypeDecl
    print(node.decl.type.args.__class__.__name__)  # ParamList
    print(node.decl.storage.__class__.__name__)  # list
    print(node.decl.quals.__class__.__name__)  # list
    print(node.decl.type.args.params[1].type.__class__.__name__)  # TypeDecl
    print(type(node))
    print(type(node).__name__)
    print(node.__class__.__name__)
    # print(node.name)
    if node.__class__.__name__ is 'FuncDef':
        print("FuncDef!!!")
    print(node.decl.type)  # 函数声明，<pycparser.c_ast.FuncDecl object at 0x000001C44D120B40>
    print(node.decl.type.args)  # 函数形参列表，<pycparser.c_ast.ParamList object at 0x0000022F0BA58EC8>
    print(node.decl.type.args.params)  # 函数形参列表内容
